        ...
        <div>
            <el-row>
                <el-col :span="24">
                    ...
                    <span style="font-size: 15px;margin-left: 10px;">新增/编辑优惠代金券</span>
                </el-col>
            </el-row>
            <br>
            <br>
    ...
    export default {
        ...
        methods: {
            fetchData(){
                ...
                this.dataApi.ajax('singlePayStoreVoucher', {voucherId: this.$route.params.id}, res => {
            ...
            submitForm(){
                ...
                if (!this.form.voucherName) {
                   this.showMsg('请输入代金券名称','warning')
                   return;
                }
                 if (!isFloat(this.form.voucherAmt)) {
                   this.showMsg('代金券价格不正确','warning')
                   return;
                }
                if (!isNumber(this.form.voucherNum)) {
                   this.showMsg('代金券库存不正确','warning')
                   return;
                }
                ...
                if (this.$route.params.id==0) {
                    ...
                    if (res.responseType === 'S') {
                        this.showMsg('新增成功')
                        this.loading=false
                        this.$router.push('/voucher/voucher_list')
                    }
                  });
                }else{
